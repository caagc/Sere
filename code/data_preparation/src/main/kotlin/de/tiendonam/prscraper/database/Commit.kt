...
interface CommitRepo : CrudRepository<Commit, Long> {
        ...
        @Query("SELECT c from Commit c WHERE c.pullRequest = :pullRequest AND c.id = :nextCommitId")
        fun findNextCommit(pullRequest: PullRequest, nextCommitId: Long): List<Commit>

        @Query("SELECT c from Commit c WHERE c.pullRequest.id = :pullRequestId ORDER BY c.createdAt")
        fun findByPullRequestId(pullRequestId: Long): List<Commit>
}